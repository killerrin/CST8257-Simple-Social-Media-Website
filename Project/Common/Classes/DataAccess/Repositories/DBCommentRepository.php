...
class DBCommentRepository extends DBGenericRepository
{
    ...
    public function getAllAuthoredByUser($userId) : array
    {
        $query = "SELECT * FROM $this->tableName
                  WHERE
                    Author_Id = '".$this->dbManager->escapeString($userId)."'";
        $result = $this->dbManager->queryCustom($query);
        return $this->parseQuery($result);
    }

    public function getAllForPicture($pictureId) : array
    {
        $query = "SELECT * FROM $this->tableName
                  WHERE
                    Picture_Id = ".$this->dbManager->escapeString($pictureId)."";
        $result = $this->dbManager->queryCustom($query);
        return $this->parseQuery($result);
    }
