...
class DBAccessibilityRepository extends DBGenericRepository
{
    ...
    function getID($key) {
        $result = $this->dbManager->queryByFilter($this->tableName, "Accessibility_Code", $dbManager->escapeString($key));
        return $this->parseQuery($result);
    }
    ...
    function insert(Accessibility $item) {
        $query = "INSERT INTO $this->tableName
                  VALUES('$dbManager->escapeString($item->Accessibility_Code)', '$dbManager->escapeString($item->Description)')";
    ...
    function update(Accessibility $item) {
        $query = "UPDATE $this->tableName
                  SET Description = '$dbManager->escapeString($item->Description)'
                  WHERE Accessibility_Code = '$dbManager->escapeString($item->Accessibility_Code)'";
        return $this->dbManager->queryCustom($query);
    }
